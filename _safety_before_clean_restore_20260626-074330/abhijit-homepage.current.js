(function () {
  ...
  const paragraphReplacements = [
    ...
    },
    {
      ...
      value: "Yes. The portfolio is planned for desktop, tablet, and mobile so recruiters and clients can scan proof quickly without losing the motion and depth of the original site."
    }
  ];
  ...
  function updateHeroMotionText() {
    ...
    Array.from(container.querySelectorAll('.eael-fancy-text-strings, .ap-hero-skill-word')).forEach(function (oldSpan) {
      try { jQuery(oldSpan).removeData(); } catch(e) {}
      oldSpan.remove();
    });

    var newSpan = document.createElement('span');
    newSpan.id = 'ap-hero-skill-word';
    newSpan.className = 'ap-hero-skill-word solid-color';
